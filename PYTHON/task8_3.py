...
x, y, r = 0, 0, 0
center_x1,center_y1=242,156
center_x2,center_y2=239,122
...
def fly_to_center(x,y,d):
    print("move")
    me.send_rc_control(int(8 * (int(x) - center_x1) / d), 0, int(8 * (center_y1 - int(y)) / d), 0)
    sleep(1)

def fly_to_center(x,y,d):
    print("move")
    me.send_rc_control(int(8 * (int(x) - center_x2) / d), 0, int(8 * (center_y2 - int(y)) / d), 0)
    sleep(1)
...
while True:
    ...
    if missionNum==0:
      ...
      d = int(math.sqrt((center_x1 - x) * (center_x1 - x) + (center_y1 - y) * (center_y1 - y)))
      print("dx=", x - center_x1)
      print("dy=", y - center_y1)

      if abs(x-center_x1) > 15 or abs(y-center_y1) > 15:
          fly_to_center(x, y, d)
      else:
          me.move_up(50)
          me.move_forward(220)
    ...
    #识别任务
      ...
      img, x, y = circle2(img)
      d = int(math.sqrt((center_x2 - x) * (center_x2 - x) + (center_y2 - y) * (center_y2 - y)))
      print("dx=", x - center_x2)
      print("dy=", y - center_y2)

      if abs(x-center_x2) > 15 or abs(y-center_y2) > 15:
        fly_to_center(x, y, d)
      else:
          me.move_forward(300)
          me.move_down(70)
